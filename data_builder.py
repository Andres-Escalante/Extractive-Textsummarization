...
def tokenize(args):
    ...
    command = ['java', '-cp' ,'../../stanford-corenlp-full-2018-10-05/*' ,'edu.stanford.nlp.pipeline.StanfordCoreNLP', '-annotators', 'tokenize,ssplit',
...
class BertData():
    ...
    def preprocess(self, src, tgt, sent_labels, use_bert_basic_tokenizer=False, is_test=False):
        ...
        if ((not is_test) and len(src) == 0):
            print("despues del 1 None");return None
        ...
        if ((not is_test) and len(src) < self.args.min_src_nsents):
            print("despues del 2 None");return None
        ...
        if ((not is_test) and len(tgt_subtoken) < self.args.min_tgt_ntokens):
            print("despues del 3 None");return None
...
def format_to_bert(args):
	...
	for corpus_type in datasets:
		...
		for json_f in glob.glob(pjoin(args.raw_path, 'news.' + corpus_type + '.*.json')):
			print(json_f)
			real_name = json_f.split('/')[-1]
			print(real_name)
			a_lst.append((corpus_type, json_f, args, pjoin(args.save_path, real_name.replace('json', 'bert.pt'))))
...
def _format_to_bert(params):
    ...
    logger.info('Processing %s' % json_file)
    jobs = json.load(open(json_file));print("len de jobs es:", len(jobs));
    datasets = [];counter=-1
    for d in jobs:
        source, tgt = d['src'], d['tgt'];counter+=1

        sent_labels = greedy_selection(source[:args.max_src_nsents], tgt, 3)
        if (args.lower):
            source = [' '.join(s).lower().split() for s in source]
            tgt = [' '.join(s).lower().split() for s in tgt]
        b_data = bert.preprocess(source, tgt, sent_labels, use_bert_basic_tokenizer=args.use_bert_basic_tokenizer, is_test=False)
        #b_data = bert.preprocess(source, tgt, sent_labels, use_bert_basic_tokenizer=args.use_bert_basic_tokenizer)
